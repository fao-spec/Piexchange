...
document.addEventListener("DOMContentLoaded", () => {
    ...
    sendMessage.addEventListener("click", () => {
        ...
        if (message) {
            ...
            setTimeout(() => addMessage("Thanks for reaching out! Please check out the Frequently Asked Questions and if you need further assistance, click on CHAT WITH US!", "bot-message"), 1000);
        }
    });
...
// You can add interactive elements or validation logic here if needed.
document.addEventListener('DOMContentLoaded', function () {
    console.log('Signup page loaded');
});

// Example conversion rates (these can be replaced with real-time API data)
const conversionRates = {
    usdt: 1.2,  // 1 Pi = 1.2 USDT
    btc: 0.0003, // 1 Pi = 0.0003 BTC
    eth: 0.01    // 1 Pi = 0.01 ETH
};

document.querySelector('.exchange-form').addEventListener('submit', function (event) {
    event.preventDefault(); // Prevent the form from submitting

    const amountInPi = parseFloat(document.getElementById('cryptoAmount').value);
    const selectedCurrency = document.getElementById('fiatCurrency').value;
    const resultElement = document.getElementById('result');

    // Validate input
    if (isNaN(amountInPi) || amountInPi <= 0) {
        resultElement.textContent = 'Please enter a valid amount in Pi.';
        resultElement.style.color = 'red'; // Set error message color
        return;
    }

    // Calculate the converted amount
    const convertedAmount = (amountInPi * conversionRates[selectedCurrency]).toFixed(2);

    // Display the result
    resultElement.textContent = `${amountInPi} Pi is approximately ${convertedAmount} ${selectedCurrency.toUpperCase()}. Kindly Sign in to connect your wallet for Pi transactions now!`;
    resultElement.style.color = '#4caf50'; // Set success message color
});
